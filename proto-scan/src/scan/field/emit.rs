...
/// [`OnScanField`] implementation that produces the read value as the event output.
pub struct EmitBytes<E: ToOwnedBytes + ?Sized>(Option<E::Owned>);
...
impl<E: ToOwnedBytes + ?Sized> ScanTypes for EmitBytes<E> {
    type ScanEvent = Infallible;
    type ScanOutput = Option<E::Owned>;
}

impl<T: ToOwnedBytes + ?Sized> EmitBytes<T> {
    pub fn new() -> Self {
        Self(None)
    }
}

impl OnScanField for EmitBytes<str> {
    fn into_output(self) -> Self::ScanOutput {
        self.0
    }
...
impl OnScanField for EmitBytes<[u8]> {
    fn into_output(self) -> Self::ScanOutput {
        self.0
    }
    fn on_scalar(&mut self, _value: ScalarField) -> Result<Option<Self::ScanEvent>, StopScan> {
